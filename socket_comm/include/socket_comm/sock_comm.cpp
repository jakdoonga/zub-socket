...
void SOCK_COMM::one_leg_exp_data()
{
    ...
    for(int j = 0; j < 2; j++)
    {
        Actual_LIFT_torque[0] |= recv_data[j+offset] << (8*j);
    }
    pub_data.act_LIFT_torque[0] = Actual_LIFT_torque[0];
    offset += 2;

    for(int j = 0; j < 4; j++)
    {
        Actual_LIFT_pos[0] |= recv_data[j+offset] << (8*j);
    }
    pub_data.act_LIFT_pos[0] = Actual_LIFT_pos[0];
    offset += 4;

    for(int j = 0; j < 4; j++)
    {
        Actual_LIFT_vel[0] |= recv_data[j+offset] << (8*j);
    }
    pub_data.act_LIFT_vel[0] = Actual_LIFT_vel[0];
}
